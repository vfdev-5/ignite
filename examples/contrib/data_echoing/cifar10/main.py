...
from utils import set_seed, get_model, get_train_test_loaders, get_train_test_loaders_with_example_echoing
...
def run(output_path, config):
    ...
    kwargs = {}
    get_loaders_fn = get_train_test_loaders

    if config["with_example_echoing"]:
        get_loaders_fn = get_train_test_loaders_with_example_echoing
        kwargs["num_echoes"] = config["num_echoes"]
        kwargs["echoing_before_dataaug"] = config["echoing_before_dataaug"]

    train_loader, test_loader, train_eval_loader = get_loaders_fn(
        path=config["data_path"], batch_size=batch_size, distributed=distributed, num_workers=num_workers, **kwargs
    )

    epoch_length = len(train_loader)
    if config["with_example_echoing"]:
        # epoch_length = (dataset_size / num_echoes + batch_size - 1) // batch_size
        epoch_length = (len(train_loader.sampler) // config["num_echoes"] + batch_size - 1) // batch_size

    model = get_model(config["model"])
    model = model.to(device)
    print("Model {}, numel={}".format(model.__class__.__name__, sum([p.numel() for p in model.parameters()])))
    ...
    try:
        trainer.run(train_loader, max_epochs=config["num_epochs"], epoch_length=epoch_length)
...
if __name__ == "__main__":
    ...
    config = {
        ...
        # Data-echoing:
        "with_example_echoing": False,
        "with_batch_echoing": False,
        "echoing_before_dataaug": True,
        "num_echoes": 3,
    }
    ...
    if config["with_example_echoing"] and config["with_batch_echoing"]:
        raise ValueError("Either with_example_echoing or with_batch_echoing should be True, but given both equal True")
