...
from ignite.metrics import MeanAveragePrecision, GeomMatchPolicy


def test_asserts():

    with pytest.raises(ValueError, match=r"Argument num_classes should be larger then one"):
        MeanAveragePrecision(num_classes=1)

    with pytest.raises(ValueError, match=r"Argument match_policy should be one of GeomMatchPolicy"):
        MeanAveragePrecision(num_classes=2, match_policy=3)

    mAP = MeanAveragePrecision(num_classes=2)
